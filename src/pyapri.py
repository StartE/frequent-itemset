...
class TranscationManager(object):
    ...
    def __init__(self, transactions):
        ...
        __num_transaction = 2
        __items = ['D3','A3','E3']
        __transaction_index_map = {'D3':{0,1},'A3':{0,1},'E3':{1}}
        """
    ...
    def add_transcation(self, transaction):
        """
        Arguments:
            transaction -- A transaction as an iterable object(e.g['D3,A3,,,,'])
        """
        for item in transaction:
            #item = 'D3,A3,,,,' in this example
            #item = ','.join([x for x in item.split(',') if x!=''])
...
RelationRecord = namedtuple(
    'RelationRecoed', SupportRecord._fields + ('ordered_statistics',))
...
def apriori(transactions, **kwargs):
    ...
    for support_record in support_records:
        ...
        if not ordered_statistics:
            continue
        yield RelationRecord(support_record.itsms, support_record.support, ordered_statistics)
...
def parse_args(argv):
    ...
    parser.add_argument(
        'input_data', metavar='inpath', nargs='*',
    ...
    args.save_output = output_funcs[args.out_format] #字典[key]
    return args
...
def dump_as_json(record, output_file):
    ...
    json.dump(
        converted_record._asdict(), output_file, default=default_func, ensure_ascii=False
    )
...
def dump_as_two_item_tsv(record, output_file):
    for ordered_stats in record.ordered_statistics:
        if len(ordered_stats.item_base) != 1:
            continue
        if len(ordered_stats.items_add) != 1:
            continue
        output_file.write('{0}\t{1}\t{2:.8f}\t{3:.9f}\t{4:.8f}{5}'.format(
...
def main(**kwargs):
    ...
    _apriori = kwargs.get('_aprioroi', apriori)

    args = _parse_args(sys.argv[1:])
    transactions = _load_transactions(
        chain(*args.input_data), delimiter=args.delimiter
    )
    result = apriori(
        transactions,
    ...
    for record in result:
        args.save_output(record, args.output)
